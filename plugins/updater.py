...
requirements_path = path.join(
    path.dirname(path.dirname(path.dirname(file))), 'requirements.txt')

HEROKU_API_KEY = config.HEROKU_API_KEY
HEROKU_APP_NAME = config.HEROKU_APP_NAME
GIT_REPO_NAME = "Đ₳Ɽ₭ Ƒմʂʂìօղ"
UPSTREAM_REPO_URL = "https://github.com/TeamFussion/DarkFussion"
...
async def upstream(ups):
    ...
    if config.HEROKU_API_KEY is not None:
        ...
        await ups.edit('`Sync Verified Successfully 🙂🙂\n'
                       'Restarting, please wait a min ,then type .alive to check if I alive !! Else Go to [Đ₳Ɽ₭ Ƒմʂʂìօղ](https://t.me/lightning_support_group)`')
    else:
